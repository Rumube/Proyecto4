...
public class SkyboxBlender : MonoBehaviour {
    ...
    void Start () {
         ...
         Materials = GetComponent<SupportSkyBox>().Materiales;
    ...
    */
    ...
    void UpdateSkyMap() 
    {
        ...
        if (Input.GetKeyDown(KeyCode.M) && i < Materials.Count)
            
        {
            i++;
            //skyBox2 = GetComponent<SupportSkyBox>().Materiales[i];
            skyBox2 = Materials[i];
            BindTextures();
            Debug.Log(i);    
        }   
        
        else if (Input.GetKeyDown(KeyCode.M) && i == Materials.Count - 1)
        {
            Debug.Log("resset");
            i = 1;
            skyBox2 = Materials[i];
